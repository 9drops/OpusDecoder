...
//    __android_log_print(ANDROID_LOG_ERROR, "HuyaDecoder", "encode_frame_size:%d", frame_size);
    int size = opus_encode(opusEncoder, pcm, frame_size, data,
                           frame_size * m_channels * sizeof(opus_int16));
//    __android_log_print(ANDROID_LOG_ERROR, "HuyaEncoder", "encode_size:%d", size);
    if (size < 0) {
//        __android_log_print(ANDROID_LOG_ERROR, "HuyaEncoder", "encode_error:%s",
//                            opus_strerror(size));
    }
    return size;
}
